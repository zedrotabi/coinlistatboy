...
const guildChannel = require("../models/voiceGuildChannel");
const userChannel = require("../models/voiceUserChannel");
const userParent = require("../models/voiceUserParent");
...
async function saveDatas(user, channel, data) {
  if (config.coin.stafs.some(x => user.member.roles.cache.has(x))) {
    if (channel.parent && config.parents.publicParents.includes(channel.parentID)) {
      if (data >= (1000 * 60) * config.coin.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: Math.floor(parseInt(data/1000/60) / config.coin.voiceCount) * config.coin.publicCoin } }, { upsert: true });
    } else if (data >= (1000 * 60) * config.coin.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: Math.floor(parseInt(data/1000/60) / config.coin.voiceCount) * config.coin.voiceCoin } }, { upsert: true });
    ...
    if (coinData && client.ranks.some(x => x.coin >= coinData.coin)) {
      ...
      if (newRank && Array.isArray(newRank.role) && !newRank.role.some(x => user.member.roles.cache.has(x)) || newRank && !Array.isArray(newRank.role) && !user.member.roles.cache.has(newRank.role)) {
        ...
        user.guild.channels.cache.get(config.channels.ranklog).send(`${user.member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve **${Array.isArray(newRank.role) ? newRank.role.map(x => `${user.guild.roles.cache.get(x).name}`).join(", ") : `${user.guild.roles.cache.get(newRank.role).name}`}** rolü verildi! :tada: :tada:`);
      }
    }
  }
